...
use super::message_model::{ClientMessage, Connect, Disconnect, Message};
use crate::db;
use crate::{
    constant::session,
    diesel::{
...
// hb方法用于处理websocket心跳机制
impl WsChatSession {
    fn hb(&self, ctx: &mut ws::WebsocketContext<Self>) {
        ctx.run_interval(session::HEARBEET, |act, ctx| {
            if Instant::now().duration_since(act.hb) > session::CLIENT_TIMEOUT {
